...
The CSV file that you downloaded contains information for just four airports, but the code works equally well with CSV files containing millions of flights and thousands of airports. Note that Pandas includes methods for loading data from SQL databases and other sources *and* for combining them into a single `DataFrame`, so your code could easily be modified to use more than a single CSV file.
