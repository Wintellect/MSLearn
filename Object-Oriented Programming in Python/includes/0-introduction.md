...
Objects combine data and code into a single entity so you can work with them as a unit. The basis for an object is a *class*, which defines the object's attributes, methods, and events. Python doesn't include native support for events, so the remainder of this module will focus on attributes and methods.
