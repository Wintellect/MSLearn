...
Specifically you want to check:
...
## SUSAN ADD LNK TO ALL_FLIGHTS.CSV
Let's start by examining the all_flights.csv file. You will need to use the data in the following columns:
...
First you need to load a pandas DataFrame with the flight data:
```python
...
The mean departure delays do not seem that large. We still need to find out the longest delays at each airport. Create a bar chart showing the maximum flight delays at each of the busy airports:
```python
...
Create a histogram of arrival delays to show how many flights were between 0-10 minutes late, 10-20 minutes late, and so on up to 120 minutes (two hours):
```python
...
Congratulations, you have now learned how to store, manipulate and visualize data in Python! You have only scratched the surface in terms of the capabilities of Python, NumPy, pandas, and Matplotlib. If you need to perform a function or create a graph that was not covered in this lesson, search for documentation and tutorials, you may discover there is a function or parameter to do exactly what you need. You are now ready to handle data in Python.
